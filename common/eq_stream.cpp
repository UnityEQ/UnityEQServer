...
#define _SID "%s: " //web session ID
#define __SID , connection_id.c_str()
...
void EQWebStream::init(bool resetSession) {
	RateThreshold=RATEBASE/250;
	DecayRate=DECAYBASE/250;
	BytesWritten=0;
	// we only reset these statistics if it is a 'new' connection
...
bool EQWebStream::HasOutgoingData()
{
	bool flag;
...
EQApplicationPacket *EQWebStream::PopPacket()
{
EQRawApplicationPacket *p=nullptr;

	MInQueue.lock();
	if (InQueue.size()) {
		std::vector<EQRawApplicationPacket *>::iterator itr=InQueue.begin();
		p=*itr;
		InQueue.erase(itr);
	}
	MInQueue.unlock();
...
EQApplicationPacket *EQWebStream::PushPacket()
{
	EQApplicationPacket *p=nullptr;

	MOutQueue.lock();
	if (OutQueue.size()) {
		std::vector<EQApplicationPacket *>::iterator itr=OutQueue.begin();
		p=*itr;
		OutQueue.erase(itr);
	}
	MOutQueue.unlock();
...
void EQWebStream::QueuePacket(const EQApplicationPacket *p, bool ack_req)
{
	if(p == nullptr)
		return;
...
void EQWebStream::FastQueuePacket(EQApplicationPacket **p, bool ack_req)
{
	EQApplicationPacket *pack=*p;
	*p = nullptr;		//clear caller's pointer.. effectively takes ownership
	...
	if(OpMgr == nullptr || *OpMgr == nullptr) {
		Log.Out(Logs::Detail, Logs::Netcode, _SID "Packet enqueued into a stream with no opcode manager, dropping." __SID);
		delete pack;
		return;
	}

	uint16 opcode = pack->emu_opcode;

	SendPacket(opcode, pack);
}


void EQWebStream::SendPacket(uint16 opcode, EQApplicationPacket *p)
{
	uint32 chunksize, used;
	uint32 length;
	...
	MOutQueue.lock();
	OutQueue.push_back(p->Copy());
	MOutQueue.unlock();

	delete p;
}

void EQWebStream::ProcessPacket(uint16 opcode, const unsigned char* data, uint32 len)
{
	EQRawApplicationPacket *ap=nullptr;
	Log.Out(Logs::Detail, Logs::Netcode, _SID "Creating new application packet, length %d" __SID, len);
	ap = new EQRawApplicationPacket(opcode, data, len);
	if(ap)
	{
		MInQueue.lock();
...
EQStream::MatchState EQWebStream::CheckSignature(const EQStream::Signature *sig, OpcodeManager* pOpcodeMgr) {
	EQRawApplicationPacket *p = nullptr;
	EQStream::MatchState res = EQStream::MatchNotReady;
...
void EQWebStream::CheckTimeout(uint32 now, uint32 timeout) {

	bool outgoing_data = HasOutgoingData();	//up here to avoid recursive locking

	EQStreamState orig_state = GetState();
	if (orig_state == CLOSING && !outgoing_data) {
		Log.Out(Logs::Detail, Logs::Netcode, _SID "Out of data in closing state, disconnecting." __SID);
		//_SendDisconnect();
		SetState(DISCONNECTING);
	} else if (LastPacket && (now-LastPacket) > timeout) {
		switch(orig_state) {
		case CLOSING:
			//if we time out in the closing state, they are not acking us, just give up
			Log.Out(Logs::Detail, Logs::Netcode, _SID "Timeout expired in closing state. Moving to closed state." __SID);
			//_SendDisconnect();
			SetState(CLOSED);
			break;
		case DISCONNECTING:
			//we timed out waiting for them to send us the disconnect reply, just give up.
			Log.Out(Logs::Detail, Logs::Netcode, _SID "Timeout expired in disconnecting state. Moving to closed state." __SID);
			SetState(CLOSED);
			break;
		case CLOSED:
			Log.Out(Logs::Detail, Logs::Netcode, _SID "Timeout expired in closed state??" __SID);
			break;
		case ESTABLISHED:
			//we timed out during normal operation. Try to be nice about it.
			//we will almost certainly time out again waiting for the disconnect reply, but oh well.
			Log.Out(Logs::Detail, Logs::Netcode, _SID "Timeout expired in established state. Closing connection." __SID);
			//_SendDisconnect();
			SetState(DISCONNECTING);
			break;
		default:
...
void EQWebStream::SetState(EQStreamState state) {
	MState.lock();
	Log.Out(Logs::Detail, Logs::Netcode, _SID "Changing state from %d to %d" __SID, State, state);
	State=state;
	MState.unlock();
}

void EQWebStream::Decay()
{
	MRate.lock();
	uint32 rate=DecayRate;
	MRate.unlock();
...
void EQWebStream::SetStreamType(EQStreamType type)
{
	Log.Out(Logs::Detail, Logs::Netcode, _SID "Changing stream type from %s to %s" __SID, StreamTypeString(StreamType), StreamTypeString(type));
	StreamType=type;
}

const char *EQWebStream::StreamTypeString(EQStreamType t)
{
	switch (t) {
		case LoginStream:
			return "Login";
...
void EQWebStream::Close() {
	if(HasOutgoingData()) {
		//there is pending data, wait for it to go out.
		Log.Out(Logs::Detail, Logs::Netcode, _SID "Stream requested to Close(), but there is pending data, waiting for it." __SID);
		SetState(CLOSING);
	} else {
		//otherwise, we are done, we can drop immediately.
		//_SendDisconnect();
		Log.Out(Logs::Detail, Logs::Netcode, _SID "Stream closing immediate due to Close()" __SID);
		SetState(DISCONNECTING);
	}
}
