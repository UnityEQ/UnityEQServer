...
class EQStream : public EQStreamInterface {
	...
	public:
		...
		virtual EQApplicationPacket *PushPacket() { return nullptr; };
		virtual std::string GetConnectionID() const { return("None"); }
...
class EQWebStream : public EQStreamInterface {
	...
	protected:
		...
		//members of this class, general purpose
		std::string connection_id; //The web interface connection identifier for this connection.
		EQStreamState State;
		Mutex MState;
		...
		bool streamactive; //have we passed our match?
		...
		//opcode related management members/functions
		OpcodeManager **OpMgr;

		//packet queues (in/out) and mutexes
		std::vector<EQApplicationPacket *> OutQueue;
		Mutex MOutQueue;
		std::vector<EQRawApplicationPacket *> InQueue;
		Mutex MInQueue;
		...
		bool Stale(uint32 now, uint32 timeout=30) { return (LastPacket && (now-LastPacket) > timeout); }
	public:
		EQWebStream() { init(); StreamType = UnknownStream;  }
		EQWebStream(std::string in_connection_id) { init(); connection_id = in_connection_id; State = ESTABLISHED; StreamType = UnknownStream;  bytes_sent = 0; bytes_recv = 0; create_time = Timer::GetTimeSeconds(); streamactive = false; active_users = 0; }
		virtual ~EQWebStream() { RemoveData(); }



		virtual uint32 GetRemoteIP() const { return 0; }
		virtual uint16 GetRemotePort() const { return 0; }
		//non-interface functions. used directly with this class throughout all projects (common)
		bool HasOutgoingData();
		bool CheckActive() { return GetState()==ESTABLISHED; }
		bool CheckClosed() { return GetState()==CLOSED; }
		...
		inline EQStreamState GetState() { EQStreamState s; MState.lock(); s=State; MState.unlock(); return s; }
		void ProcessPacket(uint16 opcode, const unsigned char* data, uint32 len);
		inline bool IsInUse() { bool flag; MInUse.lock(); flag=(active_users>0); MInUse.unlock(); return flag; }
		inline void PutInUse() { MInUse.lock(); active_users++; MInUse.unlock(); }
		void CheckTimeout(uint32 now, uint32 timeout=30);
		//interface used by application (EQStreamInterface)
		...
		virtual void FastQueuePacket(EQApplicationPacket **p, bool ack_req=true);
		virtual EQApplicationPacket *PopPacket();
		virtual EQApplicationPacket *PushPacket();
		virtual void Close();
		void Decay();
		void AdjustRates(uint32 average_delta);
		virtual void ReleaseFromUse() { MInUse.lock(); if(active_users > 0) active_users--; MInUse.unlock(); }
		virtual void RemoveData() { MOutQueue.lock(); MInQueue.lock(); OutQueue.clear(); InQueue.clear(); MInQueue.unlock(); MOutQueue.lock(); }
		virtual bool CheckState(EQStreamState state) { return GetState() == state; }
		virtual std::string Describe() const { return("Direct EQWebStream"); }
		virtual std::string GetConnectionID() const { return(connection_id); }
		uint32 bytes_sent;
		uint32 bytes_recv;
		uint32 create_time;
		...
		EQStream::MatchState CheckSignature(const EQStream::Signature *sig, OpcodeManager* pManager);
