...
#define ServerOP_WIRemoteOpcodeToClient 0x5006
#define ServerOP_WIRemoteOpcodeFromClient 0x5007
...
public:
	...
	void WriteData(const char * str, uint32 len) {memcpy(pBuffer + _wpos, str, len); _wpos += len; }

	uint8 ReadUInt8() { uint8 value = *(uint8 *)(pBuffer + _rpos); _rpos += sizeof(uint8); return value; }
	uint32 ReadUInt32() { uint32 value = *(uint32 *)(pBuffer + _rpos); _rpos += sizeof(uint32); return value; }
	void ReadString(char *str) { uint32 len = static_cast<uint32>(strlen((char *)(pBuffer + _rpos))) + 1; memcpy(str, pBuffer + _rpos, len); _rpos += len; }
	void ReadData(char *str, uint32 len) { memcpy(str, pBuffer + _rpos, len); _rpos += len; }
