...
EQStreamIdentifier::~EQStreamIdentifier() {
	...
	std::vector<WebRecord>::iterator webcur, webend;
	webcur = m_webstreams.begin();
	webend = m_webstreams.end();
	for(; webcur != webend; ++webcur) {
		WebRecord &r = *webcur;
		r.stream->ReleaseFromUse();
	}
	std::vector<WebPatch *>::iterator webcurp, webendp;
	webcurp = m_webpatches.begin();
	webendp = m_webpatches.end();
	for(; webcurp != webendp; ++webcurp) {
		delete *webcurp;
	}
}
...
void EQStreamIdentifier::RegisterWebPatch(const EQStream::Signature &sig, const char *name, OpcodeManager ** opcodes, const StructStrategy *structs) {
	WebPatch *p = new WebPatch;
	p->signature = sig;
	p->name = name;
	p->opcodes = opcodes;
	p->structs = structs;
	m_webpatches.push_back(p);
}
...
void EQStreamIdentifier::Process() {
	...
	std::vector<WebRecord>::iterator webcur;
	std::vector<WebPatch *>::iterator webcurp, webendp;
...
//				Log.LogDebugType(Logs::General, Logs::Netcode, "[IDENT_TRACE] %s:%d: Tried patch %s, but stream is not ready for it.", long2ip(r.stream->GetRemoteIP()).c_str(), ntohs(r.stream->GetRemotePort()), p->name.c_str());
	...
	while(webcur != m_webstreams.end()) {
		...
		if(r.expire.Check(false)) {
			...
			Log.Out(Logs::General, Logs::Netcode, "[IDENTIFY] Unable to identify stream from %s:%d before timeout.", long2ip(r.stream->GetRemoteIP()).c_str(), ntohs(r.stream->GetRemotePort()));
			r.stream->ReleaseFromUse();
			webcur = m_webstreams.erase(webcur);
			continue;
		}
		...
		webcurp = m_webpatches.begin();
		webendp = m_webpatches.end();
		for(; !found_one && webcurp != webendp; ++webcurp) {
			WebPatch *p = *webcurp;

			//ask the stream to see if it matches the supplied signature
			EQStream::MatchState res = r.stream->CheckSignature(&p->signature, *p->opcodes);
			switch(res) {
			case EQStream::MatchNotReady:
				//the stream has not received enough packets to compare with this signature
//				Log.LogDebugType(Logs::General, Logs::Netcode, "[IDENT_TRACE] %s:%d: Tried patch %s, but stream is not ready for it.", long2ip(r.stream->GetRemoteIP()).c_str(), ntohs(r.stream->GetRemotePort()), p->name.c_str());
...
void EQStreamIdentifier::AddStream(std::shared_ptr<EQWebStream> &eqs) {
	m_webstreams.push_back(WebRecord(eqs));
	eqs = nullptr;
}
...
EQStreamIdentifier::WebRecord::WebRecord(std::shared_ptr<EQWebStream> s)
:	stream(s),
	expire(STREAM_IDENT_WAIT_MS)
{
}