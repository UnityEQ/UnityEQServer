...
void Client::Handle_OP_ClientUpdate(const EQApplicationPacket *app)
{
	...
	if(ppu->spawn_id != GetID()) {
		...
		if (ppu->spawn_id == BoatID) {
			...
			boat->GMMove(ppu->x_pos, ppu->y_pos, ppu->z_pos, 0, false);
			return;
		}
	...
	bool pos_same = FCMP(ppu->y_pos, m_Position.y) && FCMP(ppu->x_pos, m_Position.x) && FCMP(tmpheading, m_Position.w) && ppu->rotation == animation;
