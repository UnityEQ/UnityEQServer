...
int main(int argc, char** argv) {
	...
	if (eqwsf.Open()) {
		...
		Log.Out(Logs::General, Logs::World_Server,"Failed to start WebClient (web socket based client) listener");
		return 1;
	}
	...
	std::shared_ptr<EQWebStream> eqws;
	...
	while(RunLoops) {
		...
		while ((eqws = eqwsf.Pop())) {
			...
			//structures and opcodes for that patch.
			struct in_addr	in;
			in.s_addr = eqws->GetRemoteIP();
			Log.Out(Logs::General, Logs::World_Server, "New connection from %s", eqws->GetConnectionID().c_str());
